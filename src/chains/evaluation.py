...
[평가 항목]
- relevance: 질문 의도와 평가 역량에 맞는 답변인지 평가
- specificity: 상황, 대상, 기술, 행동처럼 답변에 확인되는 구체 정보의 수준을 평가하세요. 정량 수치나 성과가 없다는 이유만으로 감점하지 마세요.
- logical_structure: 문제나 상황에서 행동과 결과로 이어지는 설명이 자연스럽고 이해하기 쉬운지 평가하세요. STAR 명칭이나 형식을 요구하지 말고, 세부 정보 부족을 이 항목에서 중복 감점하지 마세요.
- role_clarity: 지원자가 직접 맡은 역할과 기여가 명확한지 평가
- action_clarity: 수행한 행동과 판단 근거가 명확한지 평가
- result_clarity: 성과, 변화, 실패 결과 또는 학습이 명확한지 평가
...
[평가 범위]
- 각 항목은 해당 기준만 독립적으로 평가하고, 하나의 부족함을 여러 항목에서 반복 감점하지 마세요.
- INITIAL과 NEXT는 현재 답변을 중심으로 평가하세요.
- FOLLOW_UP은 동일 역량의 이전 답변과 현재 답변을 함께 평가하세요.
- MATCH, PARTIAL, UNVERIFIED 상태는 점수에 직접 반영하지 마세요.
...
answer_evaluation_prompt_v1 = _build_prompt(ANSWER_EVALUATION_SYSTEM_PROMPT_V1)
answer_evaluation_prompt = _build_prompt(ANSWER_EVALUATION_SYSTEM_PROMPT_V2)
...
def build_answer_evaluation_chain(
    ...
    *,
    ...
    if prompt_version not in {"v1", "v2"}:
        raise ValueError(f"지원하지 않는 답변 평가 prompt_version: {prompt_version}")
