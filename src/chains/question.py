...
QUESTION_GENERATION_SYSTEM_PROMPT_V2 = """
당신은 실제 면접을 진행하는 전문 면접관입니다.

[질문 유형]
- INITIAL과 NEXT는 선택된 역량을 처음 확인하는 새로운 질문을 작성하세요.
- FOLLOW_UP은 missing_points에서 가장 우선적인 한 가지만 확인하고 다른 항목을 결합하지 마세요.

[연결 상태]
- MATCH는 지원자 정보에서 확인되는 프로젝트, 조직, 기술, 역할 또는 행동 중 한 가지를 질문에 명시하고 깊게 확인하세요.
- PARTIAL은 확인된 지원자 경험 하나를 질문에 명시하고 gap_to_verify 중 한 가지만 확인하세요.
- UNVERIFIED는 관련 경험이나 JD 기술을 사용했다고 전제하지 말고 경험 유무 또는 유사 경험을 중립적으로 확인하세요.
- FOLLOW_UP에서는 연결 상태보다 직전 답변과 평가 결과를 우선하세요.
...
question_generation_prompt_v1 = _build_prompt(QUESTION_GENERATION_SYSTEM_PROMPT_V1)
question_generation_prompt = _build_prompt(QUESTION_GENERATION_SYSTEM_PROMPT_V2)
...
def build_question_generation_chain(
    ...
    *,
    ...
    if prompt_version not in {"v1", "v2"}:
        raise ValueError(f"지원하지 않는 질문 생성 prompt_version: {prompt_version}")
    ...
    prompt = (
        ...
        else question_generation_prompt
    )
