...
- `evaluators.py`: Accuracy, Macro F1, Confusion Matrix와 진단용 클래스별 F1
...
Route별 F1까지 실패 분석용으로 기록하려면 다음과 같이 실행한다.

```powershell
.\.venv\Scripts\python.exe -m evals.router.run_eval --run-langsmith --include-diagnostics
```
