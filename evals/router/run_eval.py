"""Validate or run the deterministic Router experiment."""
...
from evals.router.dataset import ensure_langsmith_dataset, load_cases
...
DEFAULT_DATASET_NAME = "askly-router-v1"
...
def validate_locally() -> list[dict]:
    ...
    for case in cases:
        ...
        expected = case["reference_outputs"]["expected_route"]
        if output["route"] != expected:
            failures.append(
                f"{case['metadata']['case_id']}: expected={expected}, "
                f"actual={output['route']}"
            )
    if failures:
        raise AssertionError("Router 검증 실패\n" + "\n".join(failures))
    return cases
